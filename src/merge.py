...
class DatasetMerger:
    """
    ZenML-safe merger.
    Accepts config but does not require it.
    """
    ...
    def merge(
        ...
        # ---- NDVI fallback
        if "NDVI_SeasonalMean" in merged.columns:
            merged["NDVI_SeasonalMean"] = merged["NDVI_SeasonalMean"].fillna(
                merged["NDVI_SeasonalMean"].median()
            )
